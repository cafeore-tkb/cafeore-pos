 ...
 */
export function generateSimpleRecommendation(
  ...
  const othersItems = items.filter(
    (item) =>
      (item.type === "others" || item.type === "milk") &&
      item.id !== toteSetsId,
  );
  ...
  // 仕様書に基づく最適化アルゴリズム
  // 実際のコーヒーアイテムを動的に取得
  const coffeeItems = Object.values(ITEM_MASTER).filter(
    (item) =>
  ...
  const itemsMap: Record<string, number> = {
    tote: toteSetsCount,
    ...Object.fromEntries(
      coffeeIds.map((id) => [id, coffeeCounts.get(id) || 0]),
    ),
  };
  ...
  if (othersItems.length > 0) {
    ...
    for (const [id, count] of othersCounts) {
