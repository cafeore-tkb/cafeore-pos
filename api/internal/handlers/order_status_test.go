...
func cupStates(order *models.Order) string {
	...
	for _, line := range order.OrderMenus {
		switch {
		case line.ServedAt != nil:
			s += "s"
		case line.ReadyAt != nil:
			s += "r"
		default:
			s += "p"
		}
...
func twoCupOrder() *models.Order {
	return &models.Order{OrderMenus: []models.OrderMenu{{ID: uuid.New()}, {ID: uuid.New()}}}
}
...
func TestToggleCupServedUpdatesOrderOnlyWhenAllCupsServed(t *testing.T) {
	...
	toggleCupServed(order, &order.OrderMenus[0], t1)
	...
	toggleCupServed(order, &order.OrderMenus[1], t2)
...
func TestToggleCupReady(t *testing.T) {
	...
	toggleCupServed(order, &order.OrderMenus[0], t2)
	toggleCupReady(order, &order.OrderMenus[0], t2)
...
func TestToggleOrderReadyAppliesToCups(t *testing.T) {
	order := twoCupOrder()
	t0, t1 := time.Now(), time.Now().Add(time.Minute)
	toggleCupReady(order, &order.OrderMenus[0], t0)
	...
	if !sameTime(order.OrderMenus[0].ReadyAt, &t0) || !sameTime(order.OrderMenus[1].ReadyAt, &t1) {
		t.Fatalf("already ready cups keep their time, others get the order's time: %+v", order.OrderMenus)
	}
...
func TestToggleOrderReadyAlwaysCancels(t *testing.T) {
	...
	t0, t1 := time.Now(), time.Now().Add(time.Minute)
	toggleCupReady(order, &order.OrderMenus[0], t0)
	toggleCupReady(order, &order.OrderMenus[1], t1)
	toggleOrderReady(order, t1)
	...
	order = twoCupOrder()
	toggleCupReady(order, &order.OrderMenus[0], t0)
	toggleCupReady(order, &order.OrderMenus[1], t0)
	toggleCupServed(order, &order.OrderMenus[0], t1)
	order.ReadyAt = &t1
	toggleOrderReady(order, t1)
...
func TestToggleOrderServedAppliesToCups(t *testing.T) {
	...
	if !sameTime(order.OrderMenus[0].ServedAt, &t1) || !sameTime(order.OrderMenus[1].ServedAt, &t2) {
		t.Fatalf("already served cups keep their time: %+v", order.OrderMenus)
	}
