 ...
 */
export const uploadAndSortOrders = async (file: File): Promise<OrderEntity[]> => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        reject(new Error("JSONファイルの解析に失敗しました。正しい形式のファイルをアップロードしてください。"));
      }
    };
 ...
 */
const createSortedFile = (orders: OrderEntity[], originalFileName: string): File => {
  ...
  return new File([sortedBlob], `sorted-${originalFileName}`, { type: "application/json" });
};
 ...
 */
export const useFileUpload = (onUploadComplete?: (orders: OrderEntity[], sortedFile: File) => void) => {
